...
def get_user():
    ...
    row = conn.execute("SELECT first_name, last_name, email, profile_image FROM users WHERE email = ?", (session["user"],)).fetchone()
    conn.close()
    if row:
...
def booking():
    if "user" not in session:
        return redirect(url_for("login"))
    user = get_user()
    return render_template("booking.html", user=user, short_name=user.get("short_name"))
...
def reservations():
    if "user" not in session:
        return redirect(url_for("login"))
    user = get_user()
    return render_template("reservations.html", user=user, short_name=user.get("short_name"))
    ...
    if request.method == "POST":
        ...
        try:
            conn.execute("INSERT INTO users (first_name, last_name, mobile, email, password) VALUES (?, ?, ?, ?, ?)",
                         (first_name, last_name, mobile, email, hashed_pw))
            conn.commit()
